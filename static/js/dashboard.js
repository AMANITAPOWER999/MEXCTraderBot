class TradingDashboard {
    ...
    updatePosition(position, currentPrice) {
        ...
        if (pnlElement && currentPrice) {
            ...
            const notional = parseFloat(position.notional || (entryPrice * size));
            let pnl;
            ...
            // Учитываем комиссию 0.03%, как в боте
            const fee = Math.abs(notional) * 0.0003;
            const pnlAfterFee = pnl - fee;
            
            pnlElement.textContent = `${pnlAfterFee >= 0 ? '+' : ''}$${pnlAfterFee.toFixed(2)} USDT`;
            pnlElement.className = pnlAfterFee >= 0 ? 'text-success fw-bold' : 'text-danger fw-bold';
        }
    }
